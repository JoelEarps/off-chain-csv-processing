use crate::{
    user_accounts_cache::errors::ErrorReportAnyhow,
    CacheHandler,
    {stream_handler::stream_generator::GenerateStream, transaction_handler::types::TxEvent},
};
...
impl<A: GenerateStream> ApplicationComponentManager<A>
where
        ...
        while let Some(record) = self.stream_task_handler.next().await {
            match record {
                Ok(transaction_event) => {
                    match self
                        .user_accounts_task_handler
                        .handle_account_update(&transaction_event)
                    {
                        Ok(_) => {
